...
class Main {
  constructor(canvas, views) {
    ...
    this.automa = new AutomaMap (
      Math.ceil(this.canvas.height / GRID),
      Math.ceil(this.canvas.width / GRID), GRID
    );
  ...
  next(input) {
    ...
    if(input === 1) {
      this.clipperMap = new ClipperMap(this.automa);
      this.interface = new Interface(this.clipperMap);
      // this.gameObjects = [this.interface, this.clipperMap];
      const a = new AutomaToClipperConverter(this.automa, GRID);
      ...
      // this.navMeshAnimator = new NavMeshAnimator(this.navMesh);
      this.gameObjects = [this.navMeshAnimator];
      this._switchView(2);
