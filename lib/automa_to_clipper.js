...
class AutomaToClipperConverter {
  ...
  _concaveman() {
    for(let i=0;i<this.seperatedPolys.length;i++) {
      let expandedPoly = this.seperatedPolys[i].map(el => [el[0] * 1000, el[1] * 1000]);
      expandedPoly = concaveman(expandedPoly, .5, 0);
      this.seperatedPolys[i] = expandedPoly.map(el => [el[0] / 1000, el[1] /1000]);
    }
  }
