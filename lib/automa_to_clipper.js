...
import concaveman from '../node_modules/concaveman/index.js';
...
class AutomaToClipperConverter {
    ...
    for(let i=0; i<this.seperatedPolys.length; i++) {
      clipperPolys[i] = this.seperatedPolys[i].map((pt) => {
          ...
          return new ClipperLib.IntPoint (
            pt[1] * this.grid + this.grid/2,
            pt[0] * this.grid + this.grid/2
          );
       });
    }
  ...
  _concaveman() {
    for(let i=0;i<this.seperatedPolys.length;i++) {
      this.seperatedPolys[i] = concaveman(this.seperatedPolys[i]);
    }
  }
