...
class ClipperMap {
  ...
  selectPrimaryMesh(pt) {
    ...
    for(let i=0;i<this.paths.length;i++) {
      if(!this.holesI[this._hash(pt)] && MeshGuru.contains(this.paths[i], pt)) {
        const area = MeshGuru.area(this.paths[i]);
        if(area > maxArea) {
          maxArea = area;
          biggestTargetIdx = this.paths[i];
        }
      }
    }
    this.biggestTargetIdx = biggestTargetIdx;
    return biggestTargetIdx === null;
  }
